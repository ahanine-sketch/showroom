...
export default function BonusSlideOver({ isOpen, onClose }: BonusSlideOverProps) {
  const [bonusAmount, setBonusAmount] = useState(2);
  const [description, setDescription] = useState('');
  
  // Mock history
  const history: BonusHistory[] = [
    { id: 1, date: '12 Mars 2024', amount: 1, description: 'Excellente performance sur la collection d\'été.' },
    { id: 2, date: '05 Jan 2024', amount: 1, description: 'Soutien exceptionnel lors de l\'inventaire.' },
  ];
    ...
    <div className="fixed inset-0 z-[100] flex justify-end font-sans">
      ...
      <div className="relative w-full max-w-lg bg-white h-full shadow-2xl animate-in slide-in-from-right duration-500 flex flex-col">
          ...
          <section className="space-y-6">
            ...
            <div className="relative pt-6">
              ...
              <div className="flex justify-between mt-4 px-1">
                {[...Array(6)].map((_, i) => (
                  <span key={i} className={`text-[10px] font-mono font-bold transition-all duration-300 ${bonusAmount === i ? 'text-yellow-600 scale-150' : 'text-stone-300'}`}>
                    {i}
                  </span>
                ))}
              </div>
              ...
              {history.map((item) => (
                <div key={item.id} className="p-5 bg-stone-50/50 border border-stone-100 rounded-2xl flex justify-between items-start group hover:bg-stone-50 hover:border-yellow-100 transition-all">
                  <div className="space-y-2">
                    <p className="text-[10px] text-stone-400 font-mono font-bold uppercase tracking-widest">{item.date}</p>
        ...
        <div className="p-8 border-t border-stone-100 bg-stone-50/50">
          <button 
            className="w-full bg-stone-900 text-white rounded-xl py-4 text-[12px] font-bold uppercase tracking-[0.2em] hover:bg-yellow-600 transition-all shadow-xl active:scale-[0.98] flex items-center justify-center gap-3 group"
          >
            Valider l'attribution
            <span className="material-symbols-outlined text-[20px] group-hover:translate-x-1 transition-transform">verified</span>
          </button>
        </div>
      </div>
    </div>
