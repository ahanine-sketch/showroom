...
exports.ShowroomScoringService = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const client_1 = require("@prisma/client");
class ShowroomScoringService {
    /**
     ...
     */
    static async calculatePerformance(showroomId, month, year) {
        ...
        const globalSettingsRaw = await prisma_1.default.globalSettings.findMany();
        ...
        if (conversionRate >= 75)
            devisPoints = pointsTB_Conv;
        else if (conversionRate >= 50)
            devisPoints = pointsB_Conv;
        else if (conversionRate >= 35)
            devisPoints = pointsM_Conv;
        ...
        if (avgBasket >= 20000)
            basketPoints = pointsTB_Basket;
        else if (avgBasket >= 15000)
            basketPoints = pointsB_Basket;
        else if (avgBasket >= 10000)
            basketPoints = pointsM_Basket;
        const totalSalesScore = caPoints + devisPoints + basketPoints;
        // --- BEHAVIOR SCORING (30 pts) ---
        // 1. AVIS (10 pts)
        const reviews = await prisma_1.default.clientReview.findMany({
            where: {
                OR: [
        ...
        if (avisNegatifs > 0)
            ...
            avisPoints = pointsTB_Avis;
        else if (avisPositifs > 0)
            avisPoints = pointsB_Avis;
        // 2. SAV (10 pts)
        ...
        if (savPlaintes > 0)
            ...
            savPoints = pointsM_SAV;
        else if (savTickets > 0)
            savPoints = pointsB_SAV;
        // 3. PROCESS (10 pts)
        ...
        if (processWarningsCount === 1)
            processPoints = pointsB_Proc;
        else if (processWarningsCount === 2)
            processPoints = pointsM_Proc;
        else if (processWarningsCount >= 3)
            processPoints = 0;
        ...
        let presenceScore = 0;
        if (totalFaults === 0)
            presenceScore = pointsTB_Pres;
        else if (totalFaults === 1)
            presenceScore = pointsB_Pres;
